...
public class CameraEffects : MonoBehaviour {
    ...
    /// Do the shake
    /// </summary>
    public static void ShakeOnce(float duration = 1f, float speed = 10f, Vector3? amount = null, Camera camera = null,
        bool deltaMovement = true, AnimationCurve curve = null) {
        //set data
        ...
        if (time > 0) {
            ...
            if (time > 0) {
                ...
                nextPos = (Mathf.PerlinNoise(time * Speed, time * Speed * 2) - 0.5f) * Amount.x * transform.right *
                          Curve.Evaluate(1f - time / Duration) +
